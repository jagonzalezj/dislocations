...
D_E = [0.2 0.2 0.5] * 1.60218e-19  % convert to joules
...
for i =1 : length(randomnum) ;% loop over the randomnumber    
    for j = 1 : length(Cint)-1; % loop over steps in Cint
        if j == length(Cint)-1;
            ...
            %plot(randomnum(i),rand(1),'r.','markersize',12)
             B(j) = B(j)+1;
           break 
        end
        
        if randomnum(i)<Cint(j+1) && randomnum(i)>Cint(j);
           %plot(randomnum(i),rand(1),'k.','markersize',12)
           %hold on
...
% %rectangle('Position', [Cint(k), 0, Cint(k+1)-Cint(k), B(k)*0.1], 'EdgeColor', 'none', 'FaceColor', colors{k}, 'LineWidth', 4);
% end
% set(gca,'ytick',[]);
