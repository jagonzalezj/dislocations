%%%%% this one uses the system of references as in the hirt and lothe 
%                  ^
%representation    | = Y;  --> = X;  .=Z
...
A = importdata('data/FEMStressSurface0_1microm.csv' );
...
for i = 1:discX;
    pos((discY*i)-(discY-1):discY*i,2) = mean(pos((discY*i)-(discY-1):discY*i,2));
end
...
% %    by their comon mean value enssuring a right organization of points
% for i = 1:discY;
%     pos((discX*i)-(discX-1):discX*i,1) = mean(pos((discX*i)-(discX-1):discX*i,1));
% end

% 3. Now sort firt by 3 and then 2 column (data fully organized)
pos = sortrows(pos, [2,1]); 

% 4. Reshape of columns to matrixes to contournf them
%    A---> micrometer     Pa----> MPa
  Y = reshape(pos(:,2),[discY,discX])'*1e-4;
  X = reshape(pos(:,1),[discY,discX])'*1e-4;
Sxx = reshape(pos(:,4),[discY,discX])'*1e-6;
Sxy = reshape(pos(:,5),[discY,discX])'*1e-6;
Sxz = reshape(pos(:,6),[discY,discX])'*1e-6;
...
MakeMaps1(Sxx, Syy, -Sxy, Szz, -X-0.5, Y, 1, [-20 20],l);
