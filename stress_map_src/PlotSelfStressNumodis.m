...
disc1 = 25;  % fem discretization in x
disc2 = 60;  % fem discretization in y
...
A = importdata('prueba.csv');

% slice the postion data
pos = [A.data(:,2) A.data(:,3) A.data(:,4) A.data(:,21:26)];
...
% Send to plot FEM Stress
  MakeMaps2(Sxx, Syy, Szz, Sxy, Sxz, Syz, Xm, Ym, 1, [-0 0]);
...
%%%-----------------------------------------------------------------------
function MakeMaps1(Sxx, Syy, Szz, Sxy, Sxz, Syz, Xm, Ym, G, param3)
    ...
    contourf(Xm,Ym, Sxx, 150, 'LineColor', 'none')
    ...
    subplot(2,2,2); 
    %imagesc(Xm(1,:), Ym(:,1), Syy); set(gca,'YDir','normal')
    contourf(Xm,Ym, Syy, 150, 'LineColor', 'none')
    ...
    subplot(2,2,3); 
    %imagesc(Xm(1,:), Ym(:,1), Szz); set(gca,'YDir','normal')
    contourf(Xm,Ym, Szz, 150, 'LineColor', 'none')
    ...
    subplot(2,2,4); 
    %imagesc(Xm(1,:), Ym(:,1), Syz); set(gca,'YDir','normal')
    contourf(Xm,Ym, Syz, 150, 'LineColor', 'none')
    ...
    set(h1,'position',[10 10 1000 1000]);
     ...
     names = {'FEM Stress', 'Self Stress','FEM + Self Stress'};

    suptitle(names(G));
    
    % print('-dpng','-r600', 'FEMStress')
