...
for i = 1:length(list)
    ...
    legend('Fem \sigma_{yz}', 'Fem \sigma_{xz}', 'Int \sigma_{yz}', 'Int \sigma_{xz}', 'Tot \sigma_{yz}', 'Tot \sigma_{xz}')
    
    print('Inf_Screw','-djpeg','-r1000'); 
...
figure(h)
    ...
    if G==2
    hold on
    plot(y,SigmaXX,'g-','linewidth', 1.0)
    plot(y,SigmaXY,'b-','linewidth', 1.0)
    ...
    plot(y,SigmaXX,'k--','linewidth', 1.0)
    plot(y,SigmaXY,'r--','linewidth', 1.0)
    %legend('T\sigma_{XX}','T\sigma_{XY}')
    end
    %hold off
    legend box off
    grid on
