...
% cd ../SelfStressInf2/;
list = dir('ResultSlab/Si_Weygand/*StressPar*.csv');
length(list);
...
for i = 1:length(list)
    ...
    for tipo =1:2
        if tipo == 1
            % Weygand
            A = importdata(['/home/gonzalez/Desktop/BENCHMARKS/Benchmarks/WEYGAND/Weygand_study2NonPBC/Si_Weygand/',num2str(list(i).name),'']);
        else
            % No Weygand
            A = importdata(['/home/gonzalez/Desktop/BENCHMARKS/Benchmarks/WEYGAND/Weygand_study2NonPBC/No_Weygand/',num2str(list(i).name),'']);
        end
